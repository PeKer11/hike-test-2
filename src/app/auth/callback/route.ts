import { NextResponse, type NextRequest } from "next/server";

import type { EmailOtpType } from "@supabase/supabase-js";

import { createClient } from "@/lib/supabase/server";

// Landing point for the link in Supabase's confirmation email.
//
// Two shapes reach us depending on the email template in use:
//   ?code=...                  — PKCE, the default for @supabase/ssr clients
//   ?token_hash=...&type=...   — the older/verify-based template
// Both are handled so the flow works without editing the email template first.
export async function GET(request: NextRequest) {
  const { searchParams, origin } = request.nextUrl;
  const code = searchParams.get("code");
  const tokenHash = searchParams.get("token_hash");
  const type = searchParams.get("type") as EmailOtpType | null;
  // Only same-origin paths — an attacker-supplied absolute URL must not be
  // able to bounce a freshly authenticated user off-site.
  const nextParam = searchParams.get("next");
  const next = nextParam?.startsWith("/") ? nextParam : "/app";

  const supabase = await createClient();

  if (code) {
    const { error } = await supabase.auth.exchangeCodeForSession(code);
    if (!error) {
      return NextResponse.redirect(`${origin}${next}`);
    }

    return redirectWithError(origin, error.message);
  }

  if (tokenHash && type) {
    const { error } = await supabase.auth.verifyOtp({
      type,
      token_hash: tokenHash,
    });
    if (!error) {
      return NextResponse.redirect(`${origin}${next}`);
    }

    return redirectWithError(origin, error.message);
  }

  return redirectWithError(origin, "Confirmation link is missing its code.");
}

function redirectWithError(origin: string, message: string) {
  const url = new URL("/login", origin);
  url.searchParams.set("error", message);

  return NextResponse.redirect(url);
}
