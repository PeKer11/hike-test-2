...
export function PlaceSearch({ onSelectPlace }: PlaceSearchProps) {
  ...
  return (
    <Card className="space-y-3">
      <div>
        <h2 className="font-display text-base font-bold text-forest">Search places</h2>
        <p className="text-xs text-charcoal/60">Search by name or address</p>
      </div>
      ...
      <input
        ...
        className="w-full rounded-md border border-charcoal/15 px-3 py-2 text-sm focus:border-terra focus:outline-none"
      />
          ...
          {results.map((result) => (
            <li key={result.id}>
              <button
                ...
                className="w-full rounded-md border border-charcoal/10 bg-cream/70 px-3 py-2 text-left text-xs text-charcoal/80 hover:bg-cream"
              >
                {result.name}
              </button>
            </li>
