    ...
    const handleBuildWalk = async (input)=>{
        ...
        try {
            ...
            if (!res.ok || data.error) {
                ...
                // Start GPS tracker + pace checker + recorder
                ...
                setIsRecording(true);
                // tick state every 30s so the UI count stays fresh
                recorder.start(()=>{
                    const update = latestPaceUpdateRef.current;
                    if (update) setRecordedPointCount((c)=>c + 1);
                    return latestPaceUpdateRef.current;
                });
...
_s(HomePage, "o9KoMd/8CuFJ4MN5+D68U5NVOsE=", false, function() {
    return [
...
//# sourceMappingURL=Documents_test_hiking-route-planner_src_be28a8b2._.js.map